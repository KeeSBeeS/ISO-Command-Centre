...
@section('content')
<style>
    .vehicle-kpi-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:14px}.vehicle-kpi{border:1px solid var(--line);border-radius:18px;padding:16px;background:rgba(15,23,42,.045);box-shadow:var(--shadow-soft)}.vehicle-kpi span{display:block;color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.08em}.vehicle-kpi strong{display:block;font-size:28px;margin-top:8px;letter-spacing:-.04em}.vehicle-dash-grid{display:grid;grid-template-columns:1.1fr .9fr;gap:14px}.mini-table table{min-width:620px}.tracking-map{height:520px;min-height:60vh;border-radius:20px;border:1px solid var(--line);overflow:hidden;background:#e9edf1;box-shadow:var(--shadow-soft)}@media(max-width:1100px){.vehicle-kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.vehicle-dash-grid{grid-template-columns:1fr}}@media(max-width:720px){.vehicle-kpi-grid{grid-template-columns:1fr}.tracking-map{height:430px;min-height:55vh}}
</style>
