    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        body{margin:0;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:
            radial-gradient(circle at top left,rgba(14,157,104,.10),transparent 30%),
            radial-gradient(circle at 82% 12%,rgba(47,111,237,.08),transparent 26%),
            linear-gradient(135deg,#f7f9fb,#eef2f5 55%,#f7f9fb);color:var(--text);min-height:100vh}
        body::before{content:"";position:fixed;inset:0;pointer-events:none;background-image:linear-gradient(rgba(15,23,42,.028) 1px,transparent 1px),linear-gradient(90deg,rgba(15,23,42,.028) 1px,transparent 1px);background-size:36px 36px;mask-image:linear-gradient(to bottom,rgba(0,0,0,.55),transparent 72%)}
        a{color:inherit;text-decoration:none}
        .app{min-height:100vh;display:grid;grid-template-columns:292px 1fr;position:relative}
        .sidebar{position:sticky;top:0;height:100vh;padding:18px;border-right:1px solid var(--line);background:linear-gradient(180deg,rgba(255,255,255,.96),rgba(248,250,252,.9));backdrop-filter:blur(18px);overflow:auto;z-index:30}
        .brand{display:flex;gap:12px;align-items:center;padding:12px 10px 22px;margin-bottom:6px}
        .brand-mark{width:46px;height:46px;border-radius:16px;background:linear-gradient(135deg,var(--brand),var(--brand2));display:grid;place-items:center;color:#071118;font-weight:950;letter-spacing:-.06em;box-shadow:0 14px 28px rgba(14,157,104,.28)}
        ...
        .nav a:hover,.nav a.active,.nav button:hover{background:rgba(15,23,42,.05);border-color:var(--line);color:var(--text);transform:translateY(-1px)}
        .nav a.active{background:linear-gradient(135deg,rgba(14,157,104,.16),rgba(56,193,114,.06));border-color:var(--line-strong);box-shadow:inset 0 0 0 1px rgba(56,193,114,.08)}
        .nav-icon{flex:0 0 32px;width:32px;height:32px;border-radius:12px;display:grid;place-items:center;background:rgba(15,23,42,.045);border:1px solid rgba(15,23,42,.08);font-size:15px;line-height:1}
        .nav a.active .nav-icon{background:linear-gradient(135deg,var(--brand),var(--brand2));color:#061017;border-color:transparent;box-shadow:0 10px 24px rgba(14,157,104,.24)}
        ...
        .main{min-width:0;position:relative}.topbar{position:sticky;top:0;z-index:20;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 22px;border-bottom:1px solid var(--line);background:rgba(255,255,255,.82);backdrop-filter:blur(18px)}
        .topbar-title{display:flex;align-items:center;gap:11px;min-width:0}.topbar-icon{width:40px;height:40px;border-radius:14px;display:grid;place-items:center;background:rgba(15,23,42,.045);border:1px solid var(--line);box-shadow:var(--shadow-soft)}.topbar h1{font-size:18px;margin:0;line-height:1.2}.topbar .sub{font-size:12px;color:var(--muted);margin-top:2px}.topbar .user{font-size:13px;color:var(--muted);text-align:right;display:flex;align-items:center;gap:10px}.user-avatar{width:38px;height:38px;border-radius:14px;background:linear-gradient(135deg,var(--panel3),var(--brand3));display:grid;place-items:center;color:#fff;font-weight:900}.user-meta strong{display:block;color:var(--text);font-size:13px}.user-meta span{display:block;color:var(--muted);font-size:11px;margin-top:1px}.menu-btn{display:none;border:1px solid var(--line);background:rgba(15,23,42,.045);color:var(--text);border-radius:13px;padding:10px 12px;align-items:center;gap:8px;font-weight:850}.sidebar-backdrop{display:none;position:fixed;inset:0;background:rgba(15,23,42,.42);z-index:25}
        ...
        .card{border:1px solid var(--line);background:linear-gradient(180deg,#ffffff,#fbfcfe);border-radius:var(--radius);box-shadow:var(--shadow);padding:18px;position:relative;overflow:hidden}.card::before{content:"";position:absolute;inset:0 0 auto;height:1px;background:linear-gradient(90deg,transparent,rgba(56,193,114,.35),transparent);pointer-events:none}.card h2,.card h3{margin-top:0;letter-spacing:-.02em}.card h2{font-size:22px}.card h3{font-size:17px}
        .metric{min-height:120px}.metric span{color:var(--muted);font-size:13px}.metric strong{display:block;font-size:34px;margin-top:10px;letter-spacing:-.04em}.metric::after{content:"";position:absolute;right:18px;top:18px;width:42px;height:42px;border-radius:15px;background:linear-gradient(135deg,rgba(14,157,104,.22),rgba(56,193,114,.08));border:1px solid rgba(56,193,114,.2)}
        .muted{color:var(--muted)}.small{font-size:13px}.actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center}.actions.right{justify-content:flex-end}.btn{border:1px solid var(--line);background:rgba(15,23,42,.04);color:var(--text);padding:11px 14px;border-radius:14px;font-weight:850;display:inline-flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease,background .18s ease,border-color .18s ease}.btn:hover{transform:translateY(-1px);background:rgba(15,23,42,.07);border-color:rgba(15,23,42,.18)}.btn.primary{background:linear-gradient(135deg,var(--brand),var(--brand3));border-color:rgba(14,157,104,.4);color:#fff;box-shadow:0 12px 26px rgba(14,157,104,.22)}.btn.primary:hover{box-shadow:0 16px 30px rgba(14,157,104,.28)}.btn.danger{background:rgba(217,54,62,.09);border-color:rgba(217,54,62,.32);color:#a01a21}.btn.full{width:100%}.btn:disabled{opacity:.55;cursor:not-allowed;transform:none}
        .table-wrap{overflow:auto;border-radius:18px;border:1px solid var(--line);box-shadow:var(--shadow-soft)}table{width:100%;border-collapse:collapse;min-width:760px;background:#fff}th,td{padding:14px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}th{color:#0b6b46;font-size:12px;text-transform:uppercase;letter-spacing:.08em;background:rgba(14,157,104,.07)}td{color:var(--text)}tbody tr{transition:background .18s ease}tbody tr:hover{background:rgba(15,23,42,.025)}tr:last-child td{border-bottom:0}.pill{display:inline-flex;align-items:center;gap:6px;border:1px solid rgba(14,157,104,.3);background:rgba(14,157,104,.09);color:#0b6b46;border-radius:999px;padding:6px 10px;font-size:12px;font-weight:850;margin:2px}.pill::before{content:"";width:6px;height:6px;border-radius:50%;background:var(--brand2);box-shadow:0 0 0 4px rgba(56,193,114,.14)}.pill.off{border-color:rgba(15,23,42,.14);color:var(--muted);background:rgba(15,23,42,.035)}.pill.off::before{background:var(--muted2);box-shadow:none}.pill.warn{border-color:rgba(184,121,10,.35);background:rgba(184,121,10,.1);color:#8a5a06}.pill.warn::before{background:var(--warn);box-shadow:0 0 0 4px rgba(184,121,10,.14)}.pill.danger{border-color:rgba(217,54,62,.35);background:rgba(217,54,62,.1);color:#a01a21}.pill.danger::before{background:var(--danger);box-shadow:0 0 0 4px rgba(217,54,62,.14)}
        label{display:block;font-size:13px;font-weight:850;margin:0 0 7px;color:#0b6b46}input,select,textarea{width:100%;border:1px solid var(--line);background:#fff;color:var(--text);border-radius:14px;padding:12px 13px;font:inherit;transition:border-color .18s ease,box-shadow .18s ease,background .18s ease}input:focus,select:focus,textarea:focus{outline:0;border-color:rgba(14,157,104,.5);box-shadow:0 0 0 4px rgba(14,157,104,.13);background:#fff}textarea{min-height:110px}option{color:#071118}.form-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.form-row{margin-bottom:14px}.checkbox-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.check{border:1px solid var(--line);background:rgba(15,23,42,.03);border-radius:14px;padding:10px;display:flex;align-items:flex-start;gap:9px}.check input{width:auto;margin-top:3px}.check:hover{border-color:rgba(14,157,104,.3);background:rgba(15,23,42,.05)}
        .alert{border-radius:16px;padding:13px 15px;margin-bottom:14px;border:1px solid var(--line);box-shadow:var(--shadow-soft)}.alert.success{background:rgba(14,157,104,.1);border-color:rgba(14,157,104,.3)}.alert.error{background:rgba(217,54,62,.1);border-color:rgba(217,54,62,.32)}.alert.warning{background:rgba(184,121,10,.1);border-color:rgba(184,121,10,.32)}.pagination{margin-top:14px}.pagination nav{display:flex;gap:8px;flex-wrap:wrap}.footer-note{padding:18px 0;color:var(--muted2);font-size:12px;text-align:center}
        .page-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:16px}.page-head-main{display:flex;gap:12px;align-items:flex-start}.page-head-icon,.section-icon{width:48px;height:48px;border-radius:17px;display:grid;place-items:center;background:linear-gradient(135deg,rgba(14,157,104,.2),rgba(56,193,114,.08));border:1px solid var(--line-strong);box-shadow:var(--shadow-soft);font-size:21px}.page-head h2{margin:0 0 5px}.page-head p{margin:0;color:var(--muted)}.soft-divider{height:1px;background:linear-gradient(90deg,transparent,var(--line),transparent);margin:16px 0}.kv-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.kv{border:1px solid var(--line);border-radius:15px;padding:11px;background:rgba(15,23,42,.025)}.kv span{display:block;color:var(--muted);font-size:12px}.kv strong{display:block;margin-top:4px}
...
<div class="app">
    ...
    <main class="main">
        ...
        <section class="content">
            ...
            @endif
            @yield('content')
            <div class="footer-note">ISO Admin Command Framework {{ \Illuminate\Support\Facades\Schema::hasTable('system_settings') ? 'v'.(\App\Models\SystemSetting::valueFor('platform_version', '2.9.1')) : 'v2.9.1' }} · isoadmin.co.za</div>
        </section>
    </main>
</div>
@auth
