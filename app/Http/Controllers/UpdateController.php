...
class UpdateController extends Controller
{
    ...
    public function v287()
    {
        ...
        return view('updates.v2_8_7', compact('systemVersion', 'permissionExists'));
    }

    public function applyV287(Request $request)
    {
        DB::transaction(function () {
            $this->seedEmployeeCompliancePermission();
            $this->seedV287Version();
        });

        $this->seedSystemAdministratorAllPermissions($request->user());

        return redirect()->route('updates.v2_8_7')->with('success', 'Version 2.8.7 applied. The Employee Compliance Overview page and dashboard widget are now available.');
    }
    ...
    private function seedV287Version(): void
    {
        if (Schema::hasTable('system_settings')) {
            DB::table('system_settings')->updateOrInsert(
                ['key' => 'platform_version'],
